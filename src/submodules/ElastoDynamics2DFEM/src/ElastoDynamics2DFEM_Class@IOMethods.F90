...
SUBMODULE(ElastoDynamics2DFEM_Class) IOMethods
...
SUBROUTINE ImportParametersFromToml(obj, table)
  ...
  func => obj%solidMaterial(1)%ptr%GetMaterialPointer("rayleigh_alpha")
  isok = ASSOCIATED(func)
  CALL AssertError1(isok, myname, "rayleigh_alpha not found")
  CALL func%GetFEVariable(obj%rayleighAlpha)
  func => NULL()

  func => obj%solidMaterial(1)%ptr%GetMaterialPointer("rayleigh_beta")
  isok = ASSOCIATED(func)
  CALL AssertError1(isok, myname, "rayleigh_beta not found")
  CALL func%GetFEVariable(obj%rayleighBeta)
  func => NULL()

  CALL obj%solidMaterial(1)%ptr%stressStrainModel%GetC(mat)
  obj%cijkl = NodalVariable(mat, &
