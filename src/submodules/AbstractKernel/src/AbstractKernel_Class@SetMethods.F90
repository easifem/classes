...
USE BoundingBox_Method, ONLY: GetDiameter
...
IF (obj%bc%isNitsche) CALL obj%SetNitscheMeshData()
...
obj%opt%lengthScale = GetDiameter(bbox)
...
obj%opt%iterData%iterationNumber = iter
...
mesh => NULL()
IF (obj%materials%ismaterialInterface) THEN
  tsize = obj%materials%tMaterialInterfaces

  DO ii = 1, tsize
    jj = obj%materials%materialInterfaces(ii)

    mesh => obj%dom%GetMeshPointer(dim=obj%opt%nsd - 1, entityNum=jj)

    CALL obj%materials%matIfaceConnectData(ii)%InitiateFacetToCellData( &
      facetMesh=mesh, cellDomain=obj%dom)
  END DO
END IF
NULLIFY (mesh)
