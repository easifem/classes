...
SUBMODULE(LagrangeSpace3D_Class) Methods
USE BaseMethod
...
INTEGER(I4B) :: ii, n
CALL AbstractPolynomialSpace3DDeallocate(obj)
...
CALL ans%Initiate( &
  ...
  & varname3=varname3, &
  & elemType=elemType)
END PROCEDURE LagrangeSpace3D1
...
CALL ans%Initiate( &
  ...
  & varname3=varname3, &
  & elemType=elemType)
END PROCEDURE LagrangeSpace3D_Pointer1
...
INTEGER(I4B) :: n, ii
  !!
CALL AbstractPolynomialSpace3DDisplay(obj=obj, msg="", unitno=unitno)
IF (ALLOCATED(obj%x)) THEN
  n = SIZE(obj%x)
