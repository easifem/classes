...
SUBMODULE(ScalarWave2DFEM_Class) AssembleMethods
...
CALL obj%tanmat%Set(VALUE=obj%diffMat, scale=scale, &
                    addContribution=.TRUE.)
...
scale = dt2
CALL obj%diffMat%matVec(y=obj%rhs, x=obj%tmp1,  &
  & addContribution=.TRUE., scale=scale)
