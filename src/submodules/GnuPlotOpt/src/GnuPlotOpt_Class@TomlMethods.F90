...
SUBMODULE(GnuPlotOpt_Class) TomlMethods
...
SUBROUTINE ImportLabelSettingsFromToml(obj, table)
  TYPE(GnuPlotLabel_), INTENT(INOUT) :: obj
  TYPE(toml_table), INTENT(INOUT) :: table
...
CONTAINS
  ...
  SUBROUTINE Help_GetAxisSettings(ax, direction)
    TYPE(GnuPlotAxis_), INTENT(INOUT) :: ax
...
SUBROUTINE ImportPlotOptsFromToml(obj, table)
  ...
  TYPE(String), ALLOCATABLE :: tmpStrVec(:)
  ...
  CALL GetValue(table=node, key="dataScale", VALUE=tmpstrVec, &
                origin=origin, stat=stat, isFound=isok)

  obj%plotOpts%scaleData = .FALSE.
  IF (isok) THEN
    obj%plotOpts%scaleData = .TRUE.
    tsize = SIZE(tmpStrVec)
    IF (tsize .EQ. 1_I4B) THEN
      obj%plotOpts%dataScale = tmpStrVec(1)
    ELSE IF (tsize .LE. 3_I4B) THEN
      obj%plotOpts%dataScale(1:tsize) = tmpStrVec(1:tsize)
    END IF
  END IF
