...
SUBMODULE(GmshPlaneSurface_Class) TomlMethods
...
USE GmshCurveLoop_Class, ONLY: GmshCurveLoopImportFromToml
IMPLICIT NONE

#ifdef DEBUG_VER
CHARACTER(*), PARAMETER :: modName = "GmshPlaneSurface_Class@TomlMethods.F90"
#endif
...
CALL ImportOuterloopFromToml(table=table, ans=outerloop)
CALL ImportInnerloopsFromToml(table=table, ans=innerloops)
CALL ImportIndxFromToml(table=table, ans=indx)

CALL obj%Initiate( &
  innerloops=innerloops, outerloop=outerloop, indx=indx)
...
SUBROUTINE ImportOuterloopFromToml(table, ans)
  CLASS(toml_table), INTENT(INOUT) :: table
  TYPE(GmshCurveLoop_), INTENT(INOUT) :: ans

#ifdef DEBUG_VER
  CHARACTER(*), PARAMETER :: myName = "ImportOuterloopFromToml()"
#endif
  ...
  CALL e%RaiseDebug(modName//'::'//myName//' - '// &
                    'Reading outerloop from toml...')
#endif

  node => NULL()
  CALL toml_get(table, "outerloop", node, origin=origin, &
                requested=math%no, stat=stat)

#ifdef DEBUG_VER
  isok = ASSOCIATED(node)
  CALL AssertError1(isok, myName, &
                    "cannot find outerloop table in config.")
#endif

  CALL ans%ImportFromToml(table=node)
  ...
  CALL e%RaiseInformation(modName//'::'//myName//' - '// &
                          '[END] ')
#endif
END SUBROUTINE ImportOuterloopFromToml

!----------------------------------------------------------------------------
!                                                   ImportInnerloopsFromToml
!----------------------------------------------------------------------------

SUBROUTINE ImportInnerloopsFromToml(table, ans)
  CLASS(toml_table), INTENT(INOUT) :: table
  TYPE(GmshCurveLoopPointer_), ALLOCATABLE, INTENT(INOUT) :: ans(:)

#ifdef DEBUG_VER
  CHARACTER(*), PARAMETER :: myName = "ImportInnerloopsFromToml()"
#endif
...
#ifdef DEBUG_VER
  CALL e%RaiseDebug(modName//'::'//myName//' - '// &
                    'Reading innerloop from toml...')
#endif

  CALL GmshCurveLoopImportFromToml( &
    obj=ans, table=table, tomlName="innerloops")

#ifdef DEBUG_VER
  CALL e%RaiseInformation(modName//'::'//myName//' - '// &
                          '[END] ')
#endif
END SUBROUTINE ImportInnerloopsFromToml
...
#define _POLYMORPHIC_TYPE_ GmshPlaneSurface_
...
#define _IMPORT_FROM_TOML_ GmshPlaneSurfaceImportFromToml
