...
CALL Initiate(obj=val, tsize=totalData)

! Get the pointer to raw data, we will fill it with data from file
vals => GetPointer(obj=val)
...
IF (abool) THEN
  ...
  DO
    ...
    IF (isok) THEN
      ...
      DO kk = 1, totalTokens
        vals(jj + kk) = tokens(kk)%To_number(val_kind)
      END DO
