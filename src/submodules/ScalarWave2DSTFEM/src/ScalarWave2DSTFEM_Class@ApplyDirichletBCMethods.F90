...
SUBMODULE(ScalarWave2DSTFEM_Class) ApplyDirichletBCMethods
...
IF (isok) THEN
  ...
  CALL obj%sol%ApplyDirichletBC(dbc=obj%dbc, &
                                times=obj%algoParam%tij(1, 1:nnt))
  CALL obj%tanmat%ApplyDBCtoRHS(x=obj%sol, y=obj%rhs, &
                                scale=minus_one, addContribution=.TRUE.)
  CALL obj%rhs%ApplyDirichletBC(dbc=obj%dbc, &
                                times=obj%algoParam%tij(1, 1:nnt))
END IF
