...
SUBMODULE(ScalarWave2DSTFEM_Class) ConstructorMethods
...
!! TODO: improve it now uniform (interpolation) order
!! in the domain is assemed
...
!   CLASS(ScalarWave2DSTFEM_), INTENT(INOUT) :: obj
...
INTEGER(I4B) :: aint, id, nnt
TYPE(String) :: matNames(3), scalarNames(6), forceName(MAX_ORDER_TIME + 1)
...
CALL Reallocate(obj%ke, aint * nnt, aint * nnt)

matNames = [String("Mass"), &
            String("Stiffness"), &
            String("Tanmat")]

CALL InitiateMatrixFields(obj=obj%matrixFields(1:2), names=matNames(1:2), &
                          matrixProps="SYM", spaceCompo=spaceCompo, &
                          timeCompo=1, fieldType=TypeField%normal, &
                          engine=default_engineName, fedof=obj%fedof)
CALL InitiateMatrixFields(obj=obj%matrixFields(3:3), names=matNames(3:3), &
                          matrixProps="UNSYM", spaceCompo=spaceCompo, &
                          timeCompo=nnt, &
...
scalarNames = [String("RHS"), String("SOL"), &
               String("RHS0"), String("U0"), &
               String("V0"), String("A0")]

CALL InitiateSTScalarFields(obj=obj%stscalarFields, names=scalarNames(1:3), &
                            fieldType=TypeField%normal, &
                            timeCompo=nnt, &
                            engine=default_engineName, fedof=obj%fedof)
CALL InitiateScalarFields(obj=obj%scalarFields, names=scalarNames(4:6), &
                          fieldType=TypeField%normal, &
                          engine=default_engineName, fedof=obj%fedof)

id = 0
id = id + 1; obj%rhs => obj%stscalarFields(id)%ptr
id = id + 1; obj%sol => obj%stscalarFields(id)%ptr
id = id + 1; obj%rhs0 => obj%stscalarFields(id)%ptr
id = 0
...
DO id = 1, nnt
  forceName(id) = String("F"//tostring(id))
END DO

CALL InitiateScalarFields(obj=obj%forceVecs(1:nnt), names=forceName(1:nnt), &
                          fieldType=TypeField%normal, &
                          engine=default_engineName, fedof=obj%fedof)
CALL InitiateScalarFields(obj=obj%tmpVecs(1:nnt), names=forceName(1:nnt), &
                          fieldType=TypeField%normal, &
                          engine=default_engineName, fedof=obj%fedof)

DO id = 1, nnt
  CALL obj%forceVecs(id)%ptr%set(VALUE=zero)
  CALL obj%tmpVecs(id)%ptr%set(VALUE=zero)
END DO
