...
SUBMODULE(ScalarWave2DSTFEM_Class) IOMethods
...
CALL obj%algoParam%Display("algoParam: ", unitno=unitno)
...
SUBROUTINE ImportEssentialsFromToml(obj, table)
  ...
  IF (debug) CALL Display(myName//" time discretization param")
  CALL obj%algoParam%ImportFromToml(table)
