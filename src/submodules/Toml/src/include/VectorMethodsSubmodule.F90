...
!
! #define _SUBMODULE_NAME_ Int8VectorMethods
! #define _METHOD_NAME_ GetValue_Int8_r1
! #define _MY_NAME_ "GetValue_Int8_r1()"
...
IF (.NOT. isFound0) &
  CALL GetVectorValue3(table, key, VALUE, origin0, stat0, isFound0, isScalar0)
...
! Define internal variables
  ...
  CASE (".csv")
    CALL GetVectorValueFromCSV(VALUE, isFound, isScalar, filename)
  CASE DEFAULT
    ! read the text file
    CALL GetVectorValueFromTXT(VALUE, isFound, isScalar, filename)
  END SELECT
...
SUBROUTINE GetVectorValueFromCSV(VALUE, isFound, isScalar, filename)
  _DATA_TYPE_, ALLOCATABLE, INTENT(INOUT) :: VALUE(:)
  LOGICAL(LGT), INTENT(INOUT) :: isFound
  LOGICAL(LGT), INTENT(INOUT) :: isScalar
...
SUBROUTINE GetVectorValueFromCSV2(VALUE, isFound, isScalar, afile)
  ...
  CALL afile%Get(2, val=tempvalvec) ! value
  ...
  DEALLOCATE (tempintvec1, tempvalvec)
...
SUBROUTINE GetVectorValueFromTXT(VALUE, isFound, isScalar, filename)
  _DATA_TYPE_, ALLOCATABLE, INTENT(INOUT) :: VALUE(:)
  LOGICAL(LGT), INTENT(INOUT) :: isFound
  LOGICAL(LGT), INTENT(INOUT) :: isScalar
  TYPE(String), INTENT(INOUT) :: filename
