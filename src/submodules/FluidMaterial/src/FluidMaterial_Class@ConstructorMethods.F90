...
SUBMODULE(FluidMaterial_Class) ConstructorMethods
USE MaterialFactory
USE FPL_Method
IMPLICIT NONE
CONTAINS
...
MODULE PROCEDURE SetFluidMaterialParam
...
END PROCEDURE SetFluidMaterialParam

!----------------------------------------------------------------------------
!                                                        CheckEssentialParam
!----------------------------------------------------------------------------

MODULE PROCEDURE Fluid_CheckEssentialParam
CHARACTER(*), PARAMETER :: myName = "Fluid_CheckEssentialParam"
...
END IF
END PROCEDURE Fluid_CheckEssentialParam

!----------------------------------------------------------------------------
!                                                                 Initiate
!----------------------------------------------------------------------------

MODULE PROCEDURE Fluid_Initiate
CHARACTER(*), PARAMETER :: myName = "Fluid_Initiate"
...
IF (ASSOCIATED(obj%stressStrainModel)) THEN
  CALL e%raiseError(modName//'::'//myName//" - "// &
    & '[CONFIG ERROR] :: The '//prefix0//'/stressStrainModel '//  &
    & CHAR_LF//'is already associated, '//  &
    & 'nullify it first.')
  RETURN
END IF
...
CALL e%RaiseInformation(modName//'::'//myName//' - '// &
  & '[END] Initiate()')
#endif
END PROCEDURE Fluid_Initiate

!----------------------------------------------------------------------------
!                                                            Deallocate
!----------------------------------------------------------------------------

MODULE PROCEDURE Fluid_Deallocate
CALL AbstractMaterialDeallocate(obj)
IF (ASSOCIATED(obj%stressStrainModel)) THEN
  DEALLOCATE (obj%stressStrainModel)
  NULLIFY (obj%stressStrainModel)
END IF
END PROCEDURE Fluid_Deallocate

!----------------------------------------------------------------------------
!                                                                    Final
!----------------------------------------------------------------------------

MODULE PROCEDURE Fluid_Final
CALL obj%DEALLOCATE()
END PROCEDURE Fluid_Final
