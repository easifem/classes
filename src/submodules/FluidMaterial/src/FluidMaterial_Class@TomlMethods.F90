...
SUBMODULE(FluidMaterial_Class) TomlMethods
...
obj%stressStrainModel => FluidMechanicsModelFactory(astr%chars())
...
MODULE PROCEDURE FluidMaterialNamesFromToml
! Internal variables
#ifdef DEBUG_VER
CHARACTER(*), PARAMETER :: myName = "FluidMaterialNamesFromToml()"
#endif
...
CALL e%RaiseInformation(modName//'::'//myName//' - '// &
                        '[END] ')
#endif
END PROCEDURE FluidMaterialNamesFromToml
...
CALL FluidMaterialNamesFromToml(table=node, materialNames=materialNames, &
                                tsize=tsize)
CALL FluidMaterialReallocate(obj, tsize)

isok = PRESENT(region)
IF (isok) CALL MeshSelectionReallocate(region, tsize)

CALL FluidMaterialImportFromToml(obj=obj, table=node, &
