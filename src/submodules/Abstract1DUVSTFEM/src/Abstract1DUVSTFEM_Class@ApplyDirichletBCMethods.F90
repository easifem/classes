...
SUBMODULE(Abstract1DUVSTFEM_Class) ApplyDirichletBCMethods
...
LOGICAL(LGT) :: isDBCLeftVelocity, isDBCRightVelocity
LOGICAL(LGT) :: isDBCLeftDisplacement, isDBCRightDisplacement
...
isDBCLeftVelocity = ASSOCIATED(obj%velocityLeft)
isDBCLeftDisplacement = ASSOCIATED(obj%displacementLeft)
isDBCRightVelocity = ASSOCIATED(obj%velocityRight)
isDBCRightDisplacement = ASSOCIATED(obj%displacementRight)
tsize_dbc_idof = 0
tsize_dbc_value = 0

IF (isDBCLeftVelocity .AND. isDBCLeftDisplacement) THEN
  ...
  DO ii = 1, nipt
    CALL obj%displacementLeft%Get(val=obj%dbc_coeff(ii), &
  ...
  CALL SymLinSolve(X=obj%dbc_value(nnt + 1:2 * nnt), A=obj%mt(1:nnt, 1:nnt), &
                   B=obj%dbc_rhs(1:nnt))

  tsize_dbc_value = tsize_dbc_value + nnt

ELSEIF (isDBCLeftVelocity .OR. isDBCLeftDisplacement) THEN
...
IF (isDBCRightVelocity .AND. isDBCRightDisplacement) THEN
  ...
  DO ii = 1, nipt
    CALL obj%displacementRight%Get(val=obj%dbc_coeff(ii), &
                                   args=obj%elemsdForTime%coord(1, ii:ii))
  END DO
