...
SUBMODULE(Abstract1DUVSTFEM_Class) SolveMethods

USE Lapack_Method, ONLY: GetInvMat, SymLinSolve, Solve
...
USE CSRMatrix_Method, ONLY: CSRMatrix_Initiate => Initiate, &
                            ...
                            CSRMatrix_convert => convert, &
                            CSRMatrix_GetDiagonal => GetDiagonal, &
                            CSRMatrix_DiagScaling => DiagonalScaling, &
                            CSRMatrixLinSolveInitiate

USE CSRMatrix_Superlu, ONLY: CSRMatrix_LUSolve => LinSolve
...
! super lu
CALL CSRMatrix_LUSolve(A=obj%tanmat, X=obj%sol%val(:), &
                       B=obj%rhs%val(:))
RETURN
