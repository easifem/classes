...
USE System_Method, ONLY: system_mkdir, RWX_U
IMPLICIT NONE
...
MODULE PROCEDURE ff_newUnitNo
  INTEGER(I4B) :: newlun,isafe
  LOGICAL(LGT) :: ostat

  newlun=MAX(MAX(stdout,stderr),stdin)+1
...
MODULE PROCEDURE ff_backspace
  ...
  IF(obj%initstat) THEN
    IF(obj%isOpen()) THEN
      BACKSPACE(UNIT=obj%unitno,IOSTAT=ioerr)
      IF(ioerr /= 0) THEN
...
MODULE PROCEDURE ff_rewind
  ...
  IF(obj%initstat) THEN
    IF(obj%isOpen()) THEN
      REWIND(UNIT=obj%unitno,IOSTAT=ioerr)
      CALL obj%setEOFstat(.FALSE.)
      IF(ioerr /= 0) THEN
