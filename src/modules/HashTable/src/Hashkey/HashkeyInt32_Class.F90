...
TYPE, EXTENDS(Hashkey_) :: HashkeyInt32_
  PRIVATE
  INTEGER(INT32) :: VALUE
CONTAINS
...
PURE FUNCTION key_equal_int32(key1, key2) RESULT(keys_equal)
  CLASS(HashkeyInt32_), INTENT(in) :: key1
...
PURE FUNCTION key_hash_int32(key) RESULT(hash)
  CLASS(HashkeyInt32_), INTENT(in) :: key
  INTEGER(INT64) :: hash
