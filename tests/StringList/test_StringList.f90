...
subroutine test1
type( Stringlist_ ) :: obj
TYPE( String )  :: data

call display( "test-1: testing add(), final(), resetIter(), nextIter()")
call obj%add( String( 1 ) )
call obj%add( String( "hello world" ) )
call obj%resetIter()
...
call obj%add( String(1) )
call obj%add( String(2) )
call obj%add( String(3) )
call obj%add( String(4) )
n = obj%size()
call ok( n == 4, 'obj%size()' )
do ii = 1, n
  call obj%getNth( ii, data )
  call ok( data .EQ. String(ii), "obj%getNth" )
end do
...
do ii = 1, obj%size()
  call obj%getNth( ii, data )
  call ok( data == string(ii), "obj%getNth" )
end do

call equalline(); call display( "popHead()"); call equalline()
call obj%popHead(data)
call ok( data == string(1), "obj%popHead()")
do ii = 1, obj%size()
  call obj%getNth( ii, data )
  call ok( data == string((ii+1)), "obj%getNth" )
end do
